...
---------- Gekonavsat mod ----------
05-09-2017 Modifications to adapt FreeIMU to Kyneo2 
----------   ----------
...
FreeIMU::FreeIMU() {
  ...
  Wire.begin();				// Added by GekoNavsat
...
float def_sea_press = 1013.25;
	...
	*/
	float FreeIMU::getBaroAlt(float sea_press) {
        ...
        //float new_press = kPress.measureRSSI(press);				//Commented by GekoNavsat
        float new_press = press;
	...
	*/
	float FreeIMU::getBaroAlt() {
		...
		float press = baro.getPressure(MS561101BA_OSR_4096);		//Added by GekoNavsat due to functional problems
...
*/
void FreeIMU::getYawPitchRoll(float * angles) {
  getEulerRad(angles);
...
//---------- Gekonavsat mod ----------
	...
	int8_t FreeIMU::setAccSF (int8_t fs_acc){

		int8_t ok = 0;
		bool mod_calib = false;
		int factor_fs_acc = 1;

		if(fs_acc == 2){		// +-2g							
			accgyro.setFullScaleAccelRange(MPU60X0_ACCEL_FS_2);
			factor_fs_acc = 1;
			mod_calib = false;	//not necessary to change calib, default
			ok = 1;
		...
		}else if(fs_acc == 16){	// +-16g	
			accgyro.setFullScaleAccelRange(MPU60X0_ACCEL_FS_16);
			factor_fs_acc = 8;
			mod_calib = true;
			ok = 1;
			...
			mod_calib = false;
			ok = 0;
			#warning Accelerometer SF not correct
			Serial.println("#Warning: Error setting Acc_SF, default settings applied");
		}


		if (mod_calib){
			acc_off_x = (float)(acc_off_x) / factor_fs_acc;
  			acc_off_y = (float)(acc_off_y) / factor_fs_acc;
  			acc_off_z = (float)(acc_off_z) / factor_fs_acc;
  			acc_scale_x = (float)(acc_scale_x) / factor_fs_acc;
  			acc_scale_y = (float)(acc_scale_y) / factor_fs_acc;
  			acc_scale_z = (float)(acc_scale_z) / factor_fs_acc;
		}

		return ok;
	}

	int8_t FreeIMU::setGyrSF (int16_t fs_gyr){

		int8_t ok = 0;
		int factor_fs_gyr = 1;

		if(fs_gyr == 250){								
			accgyro.setFullScaleGyroRange(MPU60X0_GYRO_FS_250);
			factor_fs_gyr = 8;
			gyro_sensitivity = (float)gyro_sensitivity * factor_fs_gyr;
			ok = 1;

		}else if(fs_gyr == 500){
			accgyro.setFullScaleGyroRange(MPU60X0_GYRO_FS_500);
			factor_fs_gyr = 4;
			gyro_sensitivity = (float)gyro_sensitivity * factor_fs_gyr;
			ok = 1;

		}else if(fs_gyr == 1000){
			accgyro.setFullScaleGyroRange(MPU60X0_GYRO_FS_1000);
			factor_fs_gyr = 2;
			gyro_sensitivity = (float)gyro_sensitivity * factor_fs_gyr;
			ok = 1;
			...
			factor_fs_gyr = 1;
			ok = 0;
			#warning gyroscope SF not correct
			Serial.println("#Warning: Error setting Gyr_SF, default settings applied");
		}
